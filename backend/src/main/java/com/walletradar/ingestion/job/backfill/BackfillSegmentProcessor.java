 ...
 *
 * @deprecated Replaced by {@link RawFetchSegmentProcessor} (Phase 1) + {@link ClassificationProcessor} (Phase 2).
 *             Kept for {@link #getBlockNumberFromRaw} used by ClassificationProcessor; will be removed in a follow-up.
 */
@Deprecated
@Component
@Slf4j
        ...
        while (start <= segToBlock) {
            ...
            for (RawTransaction tx : batch) {
                try {
                    ...
                    log.error("Error processing tx {} on {} for wallet {}: {}", tx.getTxHash(), networkId, walletAddress, e.getMessage(), e);
                    log.error("Raw transaction on error: txHash={}, networkId={}, rawData={}", tx.getTxHash(), tx.getNetworkId(), tx.getRawData());
                }
            }
