...
public class DeferredPriceResolutionJob {
    ...
    @Scheduled(fixedDelayString = "${walletradar.ingestion.deferred-price.schedule-interval-ms:180000}")
    public void runScheduled() {
        List<String> wallets = economicEventRepository.findDistinctWalletAddressesByFlagCode(FlagCode.PRICE_PENDING);
        for (String walletAddress : wallets) {
            resolveForWallet(walletAddress);
    ...
    public void resolveForWallet(String walletAddress) {
        ...
        for (EconomicEvent event : pending) {
            ...
            if (!result.isUnknown() && result.getPriceUsd().isPresent()) {
                ...
                event.setPriceUsd(priceUsd);
                event.setPriceSource(result.getPriceSource());
                event.setFlagCode(null);
                event.setFlagResolved(true);
                if (event.getQuantityDelta() != null) {
                    event.setTotalValueUsd(priceUsd.multiply(event.getQuantityDelta().abs()).setScale(18, RoundingMode.HALF_UP));
                }
                resolved++;
            } else {
