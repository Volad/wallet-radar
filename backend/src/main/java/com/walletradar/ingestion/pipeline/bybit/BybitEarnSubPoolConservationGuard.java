...
public class BybitEarnSubPoolConservationGuard {
    ...
    public void evaluate(List<AssetLedgerPoint> ledgerPoints) {
        ...
        List<BybitLiveBalance> liveBalances = mongoOperations.findAll(BybitLiveBalance.class);
        for (BybitLiveBalance live : liveBalances) {
            if (live == null || live.getIntegrationId() == null || live.getAssetSymbol() == null) {
                continue;
            }
            if (BybitLiveBalance.EMPTY_UMBRELLA_SYMBOL.equals(live.getAssetSymbol())) {
                continue;
            }
            String uid = extractUidFromIntegrationId(live.getIntegrationId());
            if (uid == null) {
                continue;
            }
            String symbol = live.getAssetSymbol().toUpperCase(Locale.ROOT);
            String assetKey = uid + "|" + symbol;

            BigDecimal ledgerTotal = zeroIfNull(snapshot.combinedQuantity().get(assetKey));
            BigDecimal liveTotal = sum(live.getFundQty(), live.getEarnQty(), live.getUtaQty());
