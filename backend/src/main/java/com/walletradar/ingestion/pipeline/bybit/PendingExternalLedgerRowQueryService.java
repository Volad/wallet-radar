...
public class PendingExternalLedgerRowQueryService {
    ...
    public List<ExternalLedgerRaw> loadNextBridgeRematchBatch(int batchSize) {
        int boundedBatchSize = Math.max(1, batchSize);
        Criteria matchCriteria = new Criteria().andOperator(
                Criteria.where("status").is(ExternalLedgerRawStatus.CONFIRMED),
                Criteria.where("basisRelevant").is(Boolean.TRUE),
