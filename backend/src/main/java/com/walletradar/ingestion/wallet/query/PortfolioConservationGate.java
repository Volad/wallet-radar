...
public class PortfolioConservationGate {
    ...
    /** Maximum relative amount difference to accept a bridge amount match (1.5% ≈ bridge fee). */
    private static final BigDecimal BRIDGE_PAIR_AMOUNT_TOLERANCE = new BigDecimal("0.015");
    ...
    private Set<String> buildCorridorPairedHashes(List<NormalizedTransaction> txList) {
        ...
        for (NormalizedTransaction tx : txList) {
            ...
            for (OutLeg out : outLegs) {
                ...
                if (ratio.compareTo(BRIDGE_PAIR_AMOUNT_TOLERANCE) <= 0) {
