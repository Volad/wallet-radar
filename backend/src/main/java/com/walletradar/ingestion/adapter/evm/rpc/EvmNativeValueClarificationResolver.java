...
public class EvmNativeValueClarificationResolver implements TransactionClarificationResolver {
    ...
    public Optional<ClarificationResult> clarify(NormalizedTransaction transaction) {
        ...
        if (hasInboundLeg(transaction.getFlows()) || !hasOutboundLeg(transaction.getFlows())) {
            return Optional.empty();
        }
    ...
    private static boolean hasInboundLeg(List<NormalizedTransaction.Flow> legs) {
    ...
    private static boolean hasOutboundLeg(List<NormalizedTransaction.Flow> legs) {
        return legs != null && legs.stream()
                .map(NormalizedTransaction.Flow::getQuantityDelta)
                .filter(q -> q != null)
                .anyMatch(q -> q.signum() < 0);
