...
import com.walletradar.costbasis.application.replay.state.PendingTransferStore;
 ...
 *
 * <p><b>Rollout (WS-3, open question Q1):</b> the guard is staged. {@link #SEVERITY} starts at
 * {@link Severity#WARN} so a breach emits a structured log + breach record without failing replay.
 * Promotion to {@link Severity#HARD_FAIL} (block the snapshot) is the single-line change of this
 * constant, to be flipped only after a clean full-rebuild + incremental refresh cycle confirms zero
 * orphans.
 */
@Component
@Slf4j
public class CorridorBasisConservationGuard {
    ...
    /** RC-9 D3 rollout severity. Flip to {@link Severity#HARD_FAIL} to block on orphans. */
    static final Severity SEVERITY = Severity.WARN;
    ...
    public CorridorBasisConservationResult evaluate(ReplayExecutionState replayState) {
        ...
        if (SEVERITY == Severity.HARD_FAIL) {
            throw new CorridorBasisConservationException(breaches.size(), orphanedBasisUsd);
        }
