...
public class LendingRefreshOrchestrator {
    ...
    @Qualifier(com.walletradar.config.AsyncConfig.PIPELINE_STAGE_EXECUTOR)
    private final Executor pipelineStageExecutor;

    public void triggerRefreshGroup(String sessionId, String groupId) {
        refreshStateService.markQueued(sessionId, groupId, com.walletradar.common.refresh.RefreshTrigger.MANUAL);
            ...
            for (SessionLendingQueryService.LendingGroupView group : view.groups()) {
                if ("OPEN".equals(group.status())) {
                    refreshStateService.markQueued(
                            sessionId,
                            group.id(),
                            com.walletradar.common.refresh.RefreshTrigger.BULK
                    );
                }
            }
    ...
    public List<String> openGroupIds(String sessionId) {
        return lendingQueryService.findSessionLending(sessionId)
                .map(view -> view.groups().stream()
                        ...
                        .map(SessionLendingQueryService.LendingGroupView::id)
                        .toList())
                .orElse(List.of());
    }
}
