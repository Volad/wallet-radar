...
class ReceiptClarificationGatewayTest {
    ...
    @Test
    @DisplayName("rpc-backed raw chooses rpc clarification path")
    void rpcBackedRawChoosesRpcClarificationPath() {
        ...
        when(rpcClient.call("https://rpc.example", "eth_getTransactionReceipt", List.of("0xrpc")))
                .thenReturn(Mono.just("""
                        ...
                        """));
        when(rpcTokenTransferResolver.buildTokenTransfersFromDocuments(eq("https://rpc.example"), eq(NetworkId.BSC.name()), any()))
        ...
        Optional<ClarificationReceiptEnrichment> enrichment = gateway.fetch(rawTransaction, ClarificationMode.FULL_RECEIPT);
