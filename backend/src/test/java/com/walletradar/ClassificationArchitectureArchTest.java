...
class ClassificationArchitectureArchTest {
    ...
    void family_classifiers_must_not_depend_on_jobs_or_clarification_pipeline() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..ingestion.pipeline.classification.onchain.family..")
                .should().dependOnClassesThat().resideInAnyPackage(
                        "..ingestion.job..",
                        "..ingestion.pipeline.clarification.."
                );
        rule.check(classes);
    }
    ...
    void family_classifiers_must_not_depend_on_protocol_or_special_handler_layers() {
        ArchRule protocolRule = noClasses()
                .that().resideInAPackage("..ingestion.pipeline.classification.onchain.family..")
                .should().dependOnClassesThat().resideInAnyPackage(
                        "..ingestion.pipeline.classification.onchain.protocol.registry..",
                        "..ingestion.pipeline.classification.onchain.protocol.gmx..",
                        "..ingestion.pipeline.classification.onchain.protocol.cow..",
                        "..ingestion.pipeline.classification.onchain.protocol.euler..",
                        "..ingestion.pipeline.classification.onchain.protocol.resolv..",
                        "..ingestion.pipeline.classification.special.."
                );
    ...
    void protocol_semantic_classifiers_must_not_depend_on_jobs_clarification_or_persistence() {
        ArchRule layeredRule = noClasses()
                .that().resideInAPackage("..ingestion.pipeline.classification.onchain.protocol..")
                .should().dependOnClassesThat().resideInAnyPackage(
                        "..ingestion.job..",
                        "..ingestion.pipeline.clarification..",
                        "org.springframework.data.mongodb.core..",
    ...
    void registry_adapter_layer_must_not_depend_on_special_handler_package() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..ingestion.pipeline.classification.onchain.protocol.registry..")
                .should().dependOnClassesThat().resideInAnyPackage(
                        "..ingestion.pipeline.classification.special.."
                );
        rule.check(classes);
    ...
    void clarification_services_must_not_depend_on_repositories_or_clarification_gateways() {
        ArchRule layeredRule = noClasses()
                ...
                .and().resideInAPackage("..ingestion.job.clarification..")
                .should().dependOnClassesThat().resideInAnyPackage(
                        "..ingestion.pipeline.clarification..",
                        "org.springframework.data.mongodb.core.."
                );
