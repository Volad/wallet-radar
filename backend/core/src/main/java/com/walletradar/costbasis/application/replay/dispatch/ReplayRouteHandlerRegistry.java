...
import com.walletradar.costbasis.application.replay.planning.ReplayRoute;
    ...
    static ReplayRoutingDecision decisionFor(ReplayRoute route) {
        return switch (route) {
            ...
            case LIQUID_STAKING -> ReplayRoutingDecision.liquidStaking(
                    com.walletradar.costbasis.application.replay.model.LiquidStakingFlowSelection.empty()
            );
            case FAMILY_EQUIVALENT_CUSTODY -> ReplayRoutingDecision.familyEquivalentCustody(
                    com.walletradar.costbasis.application.replay.model.SimpleFamilyCustodySelection.empty()
            );
