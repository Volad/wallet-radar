...
import com.walletradar.costbasis.application.replay.support.TransferEarnPrincipalReplaySupport;
...
public class TransferReplayHandler {
    ...
    private static final java.math.BigDecimal EARN_PRINCIPAL_DUST_BASIS_THRESHOLD =
     ...
     *
     * <p>Used for protocols that bundle collateral deposit + borrow in a single call, such as
     * INIT Capital's {@code execute(tuple _params)}. Standard AAVE-style borrows have the
     * collateral deposited in a prior separate LENDING_DEPOSIT transaction and do NOT produce
     * outbound flows in the BORROW transaction, so this path never fires for them.
     *
    ...
    private void attachLateCarryToPendingInbound(
        ...
        if (!pendingInbound.materialized()) {
            ...
            // already drained from the paired FUND/UTA source (gated on TransferEarnPrincipalReplaySupport.isBybitEarnPrincipalPaired via
