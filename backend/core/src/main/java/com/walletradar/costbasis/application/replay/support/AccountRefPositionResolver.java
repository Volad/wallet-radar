...
public final class AccountRefPositionResolver {
    ...
    /**
     * Dust-safe coverage fraction a named sub-position must hold to be treated as able to supply the
     * leg from real inventory (mirrors {@code TransferReplayHandler.CARRY_SOURCE_COVERAGE_RATIO} and
     * {@code BybitCarrySourceResolver.CARRY_SOURCE_COVERAGE_RATIO}); the tolerance absorbs sub-unit
     * fee / fill rounding while rejecting dust residues.
     */
    ...
    private static boolean positionCoversQuantity(PositionState position, BigDecimal coverQuantity) {
        ...
        return qty.compareTo(coverQuantity.multiply(CARRY_SOURCE_COVERAGE_RATIO, MC)) >= 0;
    }
}
