...
public class BybitEarnPrincipalTransferPairer {
    ...
    public static final String EARN_PRINCIPAL_CORRELATION_PREFIX = CorrelationContract.BYBIT_EARN_PRINCIPAL_V1_PREFIX;
     ...
     *
     * <p>CB-2 (corridor basis conservation orphan fix): when MORE THAN ONE candidate within the
     * skew window satisfies the equal-principal earn/non-earn opposite-sign predicate (e.g. an
     * unrelated same-quantity leg that coincidentally lands in the same window — a "decoy"), the
     * scan no longer stops at the FIRST match. It considers every in-window candidate and applies
     * {@link #isPreferredSibling} to prefer a still-unclaimed candidate over one already stamped by
     * an unrelated correlation (typically {@link BybitInternalTransferPairer}, which always runs
     * first). Absent this, a first-match-wins scan could hand the EARN leg's sibling slot to an
     * already-consumed decoy purely by list order, permanently stranding the true, still-unclaimed
     * UTA/FUND sibling (RC — CB-2, $30.19 USDC / $6.17 LDO orphans).</p>
     */
    private int pairCoEventSiblings(
        ...
        for (int i = 0; i < corridor.size(); i++) {
            ...
            for (int j = i + 1; j < corridor.size(); j++) {
                ...
                if (bestIndex == -1 || isPreferredSibling(right, corridor.get(bestIndex))) {
                    bestIndex = j;
                }
            }
            if (bestIndex == -1) {
    ...
    /**
     * CB-2 sibling-selection tiebreaker: a candidate with NO pre-existing correlation is preferred
     * over one that already carries a DIFFERENT, unrelated correlation id (e.g. a
     * {@code bybit-it-pair-v1:}/{@code bybit-it-roundtrip-v1:} id from {@link
     * BybitInternalTransferPairer}, which always runs before this pairer per {@code
     * BybitNormalizationService.processNextBatch}). An already-claimed row is not necessarily wrong
     * — if it is the ONLY candidate it is still selected — but when a genuinely free sibling is also
     * available in the same co-event window, the free one is the more trustworthy pick: it has not
     * already been committed to a different economic relationship. {@link #applyEarnPrincipalPair}
     * unconditionally overwrites whichever row is chosen, so once selection is correct no further
     * change is needed downstream.
     */
    private static boolean isPreferredSibling(NormalizedTransaction candidate, NormalizedTransaction incumbent) {
        if (isAlreadyClaimed(incumbent) && !isAlreadyClaimed(candidate)) {
            return true;
        }
        // Determinism fix: the corridor-sort fix alone only fixes iteration order, not ties among
        // 2+ still-unclaimed candidates within the same co-event skew window. When neither
        // candidate is already claimed, fall back to the lowest _id so the choice is a pure
        // function of the candidate set instead of the leaked Mongo scan order.
        if (!isAlreadyClaimed(candidate) && !isAlreadyClaimed(incumbent)) {
            return idTiebreak().compare(candidate, incumbent) < 0;
        }
        return false;
    }

    private static boolean isAlreadyClaimed(NormalizedTransaction tx) {
