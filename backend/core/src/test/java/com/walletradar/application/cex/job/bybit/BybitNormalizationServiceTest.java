...
class BybitNormalizationServiceTest {
    ...
    @Mock
    private BybitBotExecutionAttributionService bybitBotExecutionAttributionService;
    @Mock
    ...
    @Test
    void internalTransferPairerRunsBeforeEarnPrincipalPairerInBatch() {
        // CB-2 (corridor basis conservation orphan fix) ordering guard: the earn-principal
        // sibling-selection fix (BybitEarnPrincipalTransferPairer.pairCoEventSiblings) depends on
        // BybitInternalTransferPairer.repairAll() having ALREADY run — and possibly already stamped
        // bybit-it-pair-v1:/bybit-it-roundtrip-v1: correlations — before the earn-principal pairer
        // scans for co-event siblings, per the plan's documented fixed order. Nothing else in
        // BybitNormalizationService enforces this order structurally; pin it explicitly so a future
        // refactor cannot silently reorder the two passes.
        ExternalLedgerRaw row = tradeRow("trade-order-pin", "BUY");
        when(pendingExternalLedgerRowQueryService.loadNextBatch(10)).thenReturn(List.of(row));
        when(externalLedgerRawRepository.findById(row.getId())).thenReturn(Optional.of(row));
        when(bybitTradePairer.findOppositeLeg(row)).thenReturn(Optional.empty());

        BybitNormalizationService service = service();
        int processed = service.processNextBatch(10);

        assertThat(processed).isEqualTo(1);
        InOrder inOrder = Mockito.inOrder(bybitInternalTransferPairer, bybitStreamAuthorityCollapser, bybitEarnPrincipalTransferPairer);
        inOrder.verify(bybitInternalTransferPairer).repairAll();
        inOrder.verify(bybitStreamAuthorityCollapser).collapseMirrors();
        inOrder.verify(bybitEarnPrincipalTransferPairer).pairEarnPrincipalTransfers();
    }
    ...
    private BybitNormalizationService service() {
        return new BybitNormalizationService(
                ...
                bybitBotExecutionAttributionService,
