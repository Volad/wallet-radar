...
class OnChainInternalTransferPairRepairServiceTest {
    ...
    private static final String MNT_TX = "0xffc959c27972e84a0e69860e9ed312dce3db85aa6e23f2e90f22e7969b447ca1";
    private static final String MNT_SENDER = "0xa0dd42c626b002778f93e1ab42cbed5f31c117b2";
    private static final String MNT_RECEIVER = "0x1a87f12ac07e9746e9b053b8d7ef1d45270d693f";
    ...
    void setUp() {
        ...
        lenient().when(accountingUniverseService.shareUniverseMembers(MNT_SENDER, MNT_RECEIVER)).thenReturn(true);
        lenient().when(accountingUniverseService.shareUniverseMembers(MNT_RECEIVER, MNT_SENDER)).thenReturn(true);
    }

    @Test
    void repairsPairWhenBothHaveContinuityCandidateTrue() {
        NormalizedTransaction sender = internalMnt(MNT_SENDER, "-80", NormalizedLegRole.SELL);
        NormalizedTransaction receiver = internalMnt(MNT_RECEIVER, "80", NormalizedLegRole.BUY);
        ...
        String expectedCorrId = "internal-tx:mantle:" + MNT_TX;
        ...
        assertThat(sender.getCorrelationId()).isEqualTo(expectedCorrId);
        assertThat(receiver.getCorrelationId()).isEqualTo(expectedCorrId);
        assertThat(sender.getMatchedCounterparty()).isEqualTo(MNT_RECEIVER);
        assertThat(receiver.getMatchedCounterparty()).isEqualTo(MNT_SENDER);
    ...
    private static NormalizedTransaction internalMnt(String wallet, String qty, NormalizedLegRole role) {
        NormalizedTransaction tx = new NormalizedTransaction();
        tx.setId(wallet + qty);
        tx.setSource(NormalizedTransactionSource.ON_CHAIN);
        tx.setType(NormalizedTransactionType.INTERNAL_TRANSFER);
